...
async function initMenu(){
  ...
  async function loadAndRender(){
    try{
      ...
      // Merge with localStorage items as an extra safety net
      try {
        const lsItems = JSON.parse(localStorage.getItem('menu_items') || '[]');
        if (Array.isArray(lsItems) && lsItems.length) {
          ...
          lsItems.forEach(li => {
            ...
            if (!byId.has(key)) {
              ...
              // prefer latest fields from localStorage
              byId.set(key, Object.assign(byId.get(key), li));
            }
          });
      ...
      // Ensure last-added item exists and focus its category if needed
      try {
        const lastIdHint = localStorage.getItem('__last_added_id');
        if (lastIdHint) {
          const mapById = new Map(allItems.map(i => [String(i.id), i]));
          if (!mapById.has(String(lastIdHint))) {
            const lsItems = JSON.parse(localStorage.getItem('menu_items') || '[]');
            if (Array.isArray(lsItems)) {
              const candidate = lsItems.find(x => String(x.id) === String(lastIdHint));
              if (candidate) {
          ...
          if (!currentCategory) {
            const lastItem = allItems.find(x => String(x.id) === String(lastIdHint));
            if (lastItem) {
              const norm = normalizeCategory(lastItem.category);
              currentCategory = norm;
              if (categoryTabs) {
                const btn = categoryTabs.querySelector(`button[data-cat="${norm}"]`);
                if (btn) {
      ...
      // Highlight last-added item card if present
      try {
        const lastIdHint = localStorage.getItem('__last_added_id');
        if (lastIdHint && itemsGrid) {
          const link = itemsGrid.querySelector(`a[href="details.html?id=${lastIdHint}"]`);
          const card = link ? link.closest('.col-md-4') : null;
          if (card) {
            card.classList.add('border','border-warning');
            card.scrollIntoView({ behavior: 'smooth', block: 'center' });
          }
          localStorage.removeItem('__last_added_id');
        }
      } catch(err) { /* ignore */ }
  ...
  if(!initialCategory){
    try{
      const fromLast = localStorage.getItem('__last_added_category');
      if(fromLast){
        initialCategory = fromLast;
        localStorage.removeItem('__last_added_category');
...
function initCartFunctionality() {
  ...
  document.addEventListener('click', function(e) {
    if (e.target.classList.contains('add-to-cart-btn')) {
      ...
      // Highlight last added item if available
      try {
        const lastId = localStorage.getItem('__last_added_id');
        if (lastId && itemsGrid) {
          const cardLink = itemsGrid.querySelector(`a[href="details.html?id=${lastId}"]`);
